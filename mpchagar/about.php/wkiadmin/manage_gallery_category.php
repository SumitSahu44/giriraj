  ...
  if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['active'])) {
       ...
       foreach ($checkedIDs as $pageID) {
           $sql = "UPDATE tbl_gall_category SET gal_cat_status='Active' WHERE gal_cat_id='$pageID'";
      ...
      if (isset($_POST['inactive'])) {
       ...
       foreach ($checkedIDs as $pageID) {
           $sql = "UPDATE tbl_gall_category SET gal_cat_status='Inactive' WHERE gal_cat_id='$pageID'";
           $result = mysqli_query($db, $sql) or die("Query unsuccessful");
       }
    ...
    if (isset($_POST['delete'])) {
        ...
        foreach ($checkedIDs as $pageID) {
            // $existing_images_query = "SELECT `fact_icon` FROM `tbl_gall_category` WHERE gal_cat_id='$pageID'";
            ...
            $sql = "DELETE FROM `tbl_gall_category` WHERE gal_cat_id='$pageID'";
            $result = mysqli_query($db, $sql) or die("Query unsuccessful");
        }
    ...
    if (count($pageIDs) === count($site_pages_order_bys)) {
        $stmt = mysqli_prepare($db, "UPDATE tbl_gall_category SET gal_cat_order = ? WHERE gal_cat_id = ?");
        if ($stmt === false) {
    ...
    <main class="main-wrapper">
        <div class="main-content">
            ...
            <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
                <div class="breadcrumb-title pe-3">Manage Gallery Categories</div>
                <div class="ms-auto">
                  <div class="btn-group">
            ...
            <div class="row g-3">
                ...
                <div class="col-auto pb-2">
                  <div class="d-flex align-items-center gap-2 justify-content-lg-end">
                    <button class="btn btn-primary px-2" onclick="window.location.href='addedit_galcategory.php'">
                          <i class="lni lni-circle-plus"></i> Add Gallery Category</button>
                  </div>
                </div>
            </div><!--end row-->
                    ...
                    <div class="customer-table">
                        <div class="table-responsive white-space-nowrap">
                            ...
                            <table id="example" class="table table-striped table-bordered" style="width:100%">
                                ...
                                <tbody>
                                    <?php 
                                        $sql = "SELECT * FROM `tbl_gall_category` ORDER BY gal_cat_order ASC";
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo $data['gal_cat_title']?></td>
                                        <td>
                                            <input type="text" name="site_pages_order_by[]" value="<?php echo htmlspecialchars($data['gal_cat_order']); ?>" style="width:40px; outline:none;">
                                            <input type="hidden" name="paID[]" value="<?php echo htmlspecialchars($data['gal_cat_id']); ?>">
                                        </td>
                                        <td>
                                            <?php if($data["gal_cat_status"]=="Active"){?>
                                        ...
                                        <td>
                                          <a href="addedit_galcategory.php?gal_cat_id=<?php echo $data['gal_cat_id']?>"><i class="fadeIn animated bx bx-edit font-20"></i></a>
                                        </td>
                                        <td>
                                           <input type="checkbox" name="checkedIDs[]" value="<?php echo $data["gal_cat_id"]?>" class="form-check-input row-checkbox" >
                                           <input type="hidden" name="pageID[]" value="<?php echo $data["gal_cat_id"]?>">
                                        </td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
