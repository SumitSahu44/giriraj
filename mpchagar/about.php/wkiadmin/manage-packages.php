  ...
  if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['active'])) {
       ...
       foreach ($checkedIDs as $pageID) {
           $sql = "UPDATE tbl_package SET pack_status='Active' WHERE pack_id='$pageID'";
      ...
      if (isset($_POST['inactive'])) {
       ...
       foreach ($checkedIDs as $pageID) {
           $sql = "UPDATE tbl_package SET pack_status='Inactive' WHERE pack_id='$pageID'";
           $result = mysqli_query($db, $sql) or die("Query unsuccessful");
       }
    ...
    if (isset($_POST['delete'])) {
        ...
        foreach ($checkedIDs as $pageID) {
            $existing_images_query = "SELECT `pack_image` FROM `tbl_package` WHERE pack_id='$pageID'";
            ...
            if (!empty($existing_images_row['pack_image'])) {
                unlink('images/client/' . $existing_images_row['pack_image']);
            }
            
            $sql = "DELETE FROM `tbl_package` WHERE pack_id='$pageID'";
            $result = mysqli_query($db, $sql) or die("Query unsuccessful");
        }
    ...
    if (count($pageIDs) === count($site_pages_order_bys)) {
        $stmt = mysqli_prepare($db, "UPDATE tbl_package SET pack_order = ? WHERE pack_id= ?");
        if ($stmt === false) {
            ...
            <div class="row g-3">
                ...
                <div class="col-auto pb-2">
                  <div class="d-flex align-items-center gap-2 justify-content-lg-end">
                    <button class="btn btn-primary px-2" onclick="window.location.href='addedit_package.php'">
                          <i class="lni lni-circle-plus"></i> Add Package</button>
                  </div>
                </div>
            </div><!--end row-->
                    ...
                    <div class="customer-table">
                        <div class="table-responsive white-space-nowrap">
                            ...
                            <table id="example" class="table table-striped table-bordered" style="width:100%">
                                ...
                                <tbody>
                                    <?php 
                                        $sql = "SELECT * FROM `tbl_package` ORDER BY pack_order ASC";
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <img src="images/client/<?php echo $data['pack_image']?>" width="40" height="40" class="rounded-3" >
                                        </td>
                                        <td>₹<?php echo $data['pack_price']?>/ ₹<?php echo $data['pack_offer_price']?></td>
                                        <td>
                                            <input type="text" name="site_pages_order_by[]" value="<?php echo htmlspecialchars($data['pack_order']); ?>" style="width:40px; outline:none;">
                                            <input type="hidden" name="paID[]" value="<?php echo htmlspecialchars($data['pack_id']); ?>">
                                        </td>
                                        <td>
                                            <?php if($data["pack_status"]=="Active"){?>
                                        ...
                                        <td>
                                          <a href="addedit_package.php?pack_id=<?php echo $data['pack_id']?>"><i class="fadeIn animated bx bx-edit font-20"></i></a>
                                        </td>
                                        <td>
                                           <input type="checkbox" name="checkedIDs[]" value="<?php echo $data["pack_id"]?>" class="form-check-input row-checkbox" >
                                           <input type="hidden" name="pageID[]" value="<?php echo $data["pack_id"]?>">
                                        </td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
