  ...
  if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['active'])) {
       ...
       foreach ($checkedIDs as $pageID) {
           $sql = "UPDATE tbl_testimonial SET tes_status='Active' WHERE tes_id='$pageID'";
      ...
      if (isset($_POST['inactive'])) {
       ...
       foreach ($checkedIDs as $pageID) {
           $sql = "UPDATE tbl_testimonial SET tes_status='Inactive' WHERE tes_id='$pageID'";
           $result = mysqli_query($db, $sql) or die("Query unsuccessful");
       }
    ...
    if (isset($_POST['delete'])) {
        ...
        foreach ($checkedIDs as $pageID) {
            $existing_images_query = "SELECT `tes_image` FROM `tbl_testimonial` WHERE tes_id='$pageID'";
            ...
            if (!empty($existing_images_row['tes_image'])) {
                unlink('images/testimonial/' . $existing_images_row['tes_image']);
            }
            
            $sql = "DELETE FROM `tbl_testimonial` WHERE tes_id='$pageID'";
            $result = mysqli_query($db, $sql) or die("Query unsuccessful");
        }
    ...
    if (count($pageIDs) === count($site_pages_order_bys)) {
        $stmt = mysqli_prepare($db, "UPDATE tbl_testimonial SET tes_order = ? WHERE tes_id = ?");
        if ($stmt === false) {
    ...
    <main class="main-wrapper">
        <div class="main-content">
            ...
            <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
                <div class="breadcrumb-title pe-3">Manage Testimonial</div>
                <div class="ms-auto">
                </div>
            </div>
            <!--end breadcrumb-->
            <div class="row g-3">
                ...
                <div class="col-auto pb-2">
                  <div class="d-flex align-items-center gap-2 justify-content-lg-end">
                    <button class="btn btn-primary px-2" onclick="window.location.href='adddit_testimonial.php'">
                          <i class="lni lni-circle-plus"></i> Add Testimonial</button>
                  </div>
                </div>
            </div><!--end row-->
                    ...
                    <div class="customer-table">
                        <div class="table-responsive white-space-nowrap">
                            ...
                            <table id="example" class="table table-striped table-bordered" style="width:100%">
                                ...
                                <tbody>
                                    <?php 
                                        $sql = "SELECT * FROM `tbl_testimonial` ORDER BY tes_order ASC";
                                        $run = mysqli_query($db,$sql) or die("Query Not run");
                                        $count=0;
                                        while($data = mysqli_fetch_assoc($run)){
                                        $count++;
                                        $tes_rating=$data['tes_rating'];
                                    ?>
                                    <tr>
                                        <td style="display:none"><?php echo $count?></td>
                                        <td><?php echo $data['tes_name']?></td>
                                        <td>
                                            <img src="images/testimonial/<?php echo $data['tes_image']?>" width="40" height="40" class="rounded-3" >
                                        </td>
                                        <td><?php if ($tes_rating == 5) echo '
                                        ...
                                        <?php if ($tes_rating == 3) echo '
                                        <label class="form-check-label" for="flexRadioSuccess">
                                            <i class="lni lni-star-filled "></i>
                                            <i class="lni lni-star-filled "></i>
                                            <i class="lni lni-star-filled "></i>
                                        </label>';?>
                                        <?php if ($tes_rating == 2) echo '
                                        <label class="form-check-label" for="flexRadioSuccess">
                                            <i class="lni lni-star-filled "></i>
                                            <i class="lni lni-star-filled "></i>
                                        </label>';?>
                                        <?php if ($tes_rating == 1) echo '
                                        <label class="form-check-label" for="flexRadioSuccess">
                                            <i class="lni lni-star-filled "></i>
                                        </label>';?>
                                        </td>
                                        <td>
                                            <input type="text" name="site_pages_order_by[]" value="<?php echo htmlspecialchars($data['tes_order']); ?>" style="width:40px; outline:none;">
                                            <input type="hidden" name="paID[]" value="<?php echo htmlspecialchars($data['tes_id']); ?>">
                                        </td>
                                        <td>
                                            <?php if($data["tes_status"]=="Active"){?>
                                        ...
                                        <td>
                                          <a href="adddit_testimonial.php?tes_id=<?php echo $data['tes_id']?>"><i class="fadeIn animated bx bx-edit font-20"></i></a>
                                        </td>
                                        <td>
                                           <input type="checkbox" name="checkedIDs[]" value="<?php echo $data["tes_id"]?>" class="form-check-input row-checkbox" >
                                           <input type="hidden" name="pageID[]" value="<?php echo $data["tes_id"]?>">
                                        </td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
