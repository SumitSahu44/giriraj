  ...
  if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['active'])) {
       ...
       foreach ($checkedIDs as $pageID) {
           $sql = "UPDATE tbl_external_links SET link_status='Active' WHERE link_id='$pageID'";
      ...
      if (isset($_POST['inactive'])) {
       ...
       foreach ($checkedIDs as $pageID) {
           $sql = "UPDATE tbl_external_links SET link_status='Inactive' WHERE link_id='$pageID'";
           $result = mysqli_query($db, $sql) or die("Query unsuccessful");
       }
    ...
    if (isset($_POST['delete'])) {
        ...
        foreach ($checkedIDs as $pageID) {
            ...
            $sql = "DELETE FROM `tbl_external_links` WHERE link_id='$pageID'";
            $result = mysqli_query($db, $sql) or die("Query unsuccessful");
        }
    ...
    if (count($pageIDs) === count($site_pages_order_bys)) {
        $stmt = mysqli_prepare($db, "UPDATE tbl_external_links SET link_order = ? WHERE link_id = ?");
        if ($stmt === false) {
        ...
        header("Location: manage-externallinks.php");
        exit();
    } else {
    ...
    <main class="main-wrapper">
        <div class="main-content">
            ...
            <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
                <div class="breadcrumb-title pe-3">Manage External Links</div>
            ...
            <div class="row g-3">
                ...
                <div class="col-auto pb-2">
                  <div class="d-flex align-items-center gap-2 justify-content-lg-end">
                    <button class="btn btn-primary px-2" onclick="window.location.href='addedit_externallink.php'">
                          <i class="lni lni-circle-plus"></i> Add External Link</button>
                  </div>
                </div>
            </div><!--end row-->
                    ...
                    <div class="customer-table">
                        <div class="table-responsive white-space-nowrap">
                            ...
                            <table id="example" class="table table-striped table-bordered" style="width:100%">
                                ...
                                <tbody>
                                    <?php 
                                        $sql = "SELECT * FROM `tbl_external_links` ORDER BY link_order ASC";
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="text" name="site_pages_order_by[]" value="<?php echo htmlspecialchars($data['link_order']); ?>" style="width:40px; outline:none;">
                                            <input type="hidden" name="paID[]" value="<?php echo htmlspecialchars($data['link_id']); ?>">
                                        </td>
                                        <td>
                                            <?php if($data["link_status"]=="Active"){?>
                                        ...
                                        <td>
                                          <a href="addedit_externallink.php?link_id=<?php echo $data['link_id']?>"><i class="fadeIn animated bx bx-edit font-20"></i></a>
                                        </td>
                                        <td>
                                           <input type="checkbox" name="checkedIDs[]" value="<?php echo $data["link_id"]?>" class="form-check-input row-checkbox" >
                                           <input type="hidden" name="pageID[]" value="<?php echo $data["link_id"]?>">
                                        </td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
