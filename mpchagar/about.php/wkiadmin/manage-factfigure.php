  ...
  if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['active'])) {
       ...
       foreach ($checkedIDs as $pageID) {
           $sql = "UPDATE tbl_facts SET fact_status='Active' WHERE fact_id='$pageID'";
      ...
      if (isset($_POST['inactive'])) {
       ...
       foreach ($checkedIDs as $pageID) {
           $sql = "UPDATE tbl_facts SET fact_status='Inactive' WHERE fact_id='$pageID'";
           $result = mysqli_query($db, $sql) or die("Query unsuccessful");
       }
    ...
    if (isset($_POST['delete'])) {
        ...
        foreach ($checkedIDs as $pageID) {
            $existing_images_query = "SELECT `fact_icon` FROM `tbl_facts` WHERE fact_id='$pageID'";
            ...
            if (!empty($existing_images_row['fact_icon'])) {
                unlink('images/client/' . $existing_images_row['fact_icon']);
            }
            
            $sql = "DELETE FROM `tbl_facts` WHERE fact_id='$pageID'";
            $result = mysqli_query($db, $sql) or die("Query unsuccessful");
        }
    ...
    if (count($pageIDs) === count($site_pages_order_bys)) {
        $stmt = mysqli_prepare($db, "UPDATE tbl_facts SET fact_order = ? WHERE fact_id = ?");
        if ($stmt === false) {
        ...
        header("Location: manage-factfigure.php");
        exit();
    } else {
    ...
    <main class="main-wrapper">
        <div class="main-content">
            ...
            <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
                <div class="breadcrumb-title pe-3">Manage Fact & Figure</div>
                <div class="ms-auto">
                </div>
            </div>
            <!--end breadcrumb-->
            <div class="row g-3">
                ...
                <div class="col-auto pb-2">
                  <div class="d-flex align-items-center gap-2 justify-content-lg-end">
                    <button class="btn btn-primary px-2" onclick="window.location.href='addedit_factfigure.php'">
                          <i class="lni lni-circle-plus"></i> Add Fact & Figure</button>
                  </div>
                </div>
            </div><!--end row-->
                    ...
                    <div class="customer-table">
                        <div class="table-responsive white-space-nowrap">
                            ...
                            <table id="example" class="table table-striped table-bordered" style="width:100%">
                                ...
                                <tbody>
                                    <?php 
                                        $sql = "SELECT * FROM `tbl_facts` ORDER BY fact_order ASC";
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo $data['fact_heading']?></td>
                                        <td>
                                            <img src="images/client/<?php echo $data['fact_icon']?>" width="40" height="40" class="rounded-3" >
                                        </td>
                                        <td>
                                            <input type="text" name="site_pages_order_by[]" value="<?php echo htmlspecialchars($data['fact_order']); ?>" style="width:40px; outline:none;">
                                            <input type="hidden" name="paID[]" value="<?php echo htmlspecialchars($data['fact_id']); ?>">
                                        </td>
                                        <td>
                                            <?php if($data["fact_status"]=="Active"){?>
                                        ...
                                        <td>
                                          <a href="addedit_factfigure.php?fact_id=<?php echo $data['fact_id']?>"><i class="fadeIn animated bx bx-edit font-20"></i></a>
                                        </td>
                                        <td>
                                           <input type="checkbox" name="checkedIDs[]" value="<?php echo $data["fact_id"]?>" class="form-check-input row-checkbox" >
                                           <input type="hidden" name="pageID[]" value="<?php echo $data["fact_id"]?>">
                                        </td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
